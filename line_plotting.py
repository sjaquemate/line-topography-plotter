...
def plot_segment_3d(ax, segment, segment_width, segment_color, alpha=1):
    l_i, = ax.plot(segment[:, 0], segment[:, 1], segment[:, 2], linewidth=segment_width, c='black', alpha=alpha)
...
def plot_line_3d(ax, x, y, z, linewidths, linecolors):
    segments = get_segments(x, y, z)
    for i in range(len(segments)):
        plot_segment_3d(ax, segments[i], linewidths[i], linecolors[i])